...
export const Projects: React.FC = () => {
  ...
  const filteredProjects = PORTFOLIO_DATA.projects.filter(project => {
    ...
    return project.category === selectedCategory || project.technologies.includes(selectedCategory);
  });
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-3xl mx-auto mb-12">
          ...
          <p className="text-slate-600 dark:text-slate-300 text-base sm:text-lg">
            Explore interactive dashboards, DAX calculations, and data analytics case studies developed to solve real-world corporate challenges.
          </p>
        </div>
          ...
          <AnimatePresence>
            {filteredProjects.map((project) => (
              <ProjectCard
                key={project.id}
                project={project}
                onOpenCaseStudy={(p) => setActiveCaseStudyProject(p)}
              />
            ))}
          </AnimatePresence>
        </div>
